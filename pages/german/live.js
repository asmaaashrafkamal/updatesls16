...
var tbody = document.getElementById('tbody');
    ...
    function test4() {
        findUserByEmail1(function (arr,all_id) {
            ...
            var post_live=[]
        if(arr.length==0){
            document.getElementById("emp").innerHTML="<h3 style='text-align:center;color:gold;'>Kein Verkäufer/Administrator gefunden</h3>"
        }else{
        ...
        .then((querySnapshot1) =>{
            if (!querySnapshot1.empty) {
                ...
                for (let i = 0; i < all_id.length; i++) {   
                    if(doc1.data().uid==all_id[i]){
                        ...
                        tr += "<tr><td><img src="+ arr[i]['profileImg'] +" width=40% height=30%/></td><td>" + arr[i]['userName']+ "</td><td>" +arr[i]['email'] + "</td><td><h6  >in Live jetzt</h6></td></tr>"; 
                    }
                }
                console.log(post_live)
            }
            ...
            document.getElementById("emp").innerHTML="<h3 style='text-align:center;color:gold;'>Niemand in Live jetzt</h3>"
        }
        console.log(post_live)
