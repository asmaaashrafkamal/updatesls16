...
var tbody = document.getElementById('tbody');
    ...
    function test4() {
        findUserByEmail1("SuperUser" ,function (arr) {
            // console.log(arr[0]['email']);
            var tr ='';
            var element='';
            //console.log(arr);
            if(arr.length==0){
                document.getElementById("emp").innerHTML="<h3 style='text-align:center;color:gold;'>Kein Administrator gefunden</h3>"
    
            // tr1=""
            }else{
            for (let i = 0; i < arr.length; i++) {    
            tr += "<tr><td><img src="+ arr[i]['profileImg'] +" width=40% height=30%/></td><td>" + arr[i]['userName']+ "</td><td>" +arr[i]['email'] + "</td><td><a  id='profile"+i+ "' href=''>Zeige Profil</a></td><td><a  id='getid"+i+ "' href=''>alle Bestellungen bekommen</a></td></tr>";
           ...
           //  element=doc.id;
            ...
            const database=firebase.firestore();
               // element=doc.id;
                ...
                console.log(document.getElementById("getid"+i));
                window.location.href = "get_user_order.html?id="+doc.id
            })
           ...
           console.log(arr);
            ...
            for (let i = 0; i < arr.length; i++) {    
             if(arr[i]['userName'].toLowerCase().startsWith(search_name.toLowerCase())){
                tr1 += "<tr><td><img src="+ arr[i]['profileImg'] +" width=40% height=30%/></td><td>" + arr[i]['userName']+ "</td><td>" +arr[i]['email'] + "</td><td><a  id='profile"+i+ "' href=''>Zeige Profil</a></td><td><a  id='getid"+i+ "' href=''>alle Bestellungen bekommen</a></td></tr>";
                database.collection("Users").where("email", "==", arr[i]['email']).get()
                .then((querySnapshot) =>{
                querySnapshot.forEach((doc) => {
               //  document.getElementById('getid'+i).value =doc.id;
