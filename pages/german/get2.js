    ...
    function test4(id) {
        ...
        for(let i=0;i<arr.length;i++){
            ...
            if((arr[i]['orderState']=="Pending" ||arr[i]['orderState']=="Accepted")){
                tr1 += "<tr><td><img src='' id='im"+i+ "'  width=40% height=30%/></td><td id='name"+i+ "'></td><td>" + arr[i]['orderNo']+ "</td><td>Ausstehend</td><td>" +arr[i]['orderTotalPrice'] + "€</td><td><form><div  style='width:200px;'><select  id='sel"+i+ "' ><option disabled selected hidden>Select State:</option><option value='1'>Ausstehend</option><option value='2'>Versand</option><option value='3'>abgesagt</option></select></div> <br><br>"
             ...
             if((arr[i]['orderState']=="Shipped")){
                tr1 += "<tr><td><img src='' id='im"+i+ "'  width=40% height=30%/></td><td id='name"+i+ "'></td><td>" + arr[i]['orderNo']+ "</td><td>Versand</td><td>" +arr[i]['orderTotalPrice'] + "€</td><td><form><div  style='width:200px;'><select  id='sel"+i+ "' ><option disabled selected hidden>Select State:</option><option value='1'>Ausstehend</option><option value='2'>Versand</option><option value='3'>abgesagt</option></select></div> <br><br>"
               tr1+="<button type='submit' style='text-align:center;' class='btn btn-danger' value=''  id='getid"+i+ "' name='getid' >Change State</button></form></td></tr>";
             }
             if((arr[i]['orderState']=="Canceled")){
                tr1 += "<tr><td><img src='' id='im"+i+ "'  width=40% height=30%/></td><td id='name"+i+ "'></td><td>" + arr[i]['orderNo']+ "</td><td>abgesagt</td><td>" +arr[i]['orderTotalPrice'] + "€</td><td><form><div  style='width:200px;'><select  id='sel"+i+ "' ><option disabled selected hidden>Select State:</option><option value='1'>Ausstehend</option><option value='2'>Versand</option><option value='3'>abgesagt</option></select></div> <br><br>"
               tr1+="<button type='submit' style='text-align:center;' class='btn btn-danger' value=''  id='getid"+i+ "' name='getid' >Change State</button></form></td></tr>";
             }
             if(tr1 !=''){
                console.log("tooo")
