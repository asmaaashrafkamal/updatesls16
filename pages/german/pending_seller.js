...
const user=database.collection('Users');
    ...
    function test4() {
        findUserByEmail1("PendingSeller" ,function (arr) {
            ...
            var element='';
            if(arr.length==0){
                document.getElementById("emp").innerHTML="<h3 style='text-align:center;color:gold;'>Kein ausstehender Verkäufer gefunden</h3>"
            }else{
            for (let i = 0; i < arr.length; i++) {    
            tr += "<tr><td>" + arr[i]['userName']+ "</td><td>" +arr[i]['email'] + "</td><td>" +arr[i]['confirmState'] + "</td><td>" +arr[i]['role'] + "</td><td><form><button type='submit' style='text-align:center;' class='btn btn-danger' value=''  id='delete"+i+ "' name='delete' >Als Verkäufer bestätigen</button></form></td><td><form><button type='submit' style='text-align:center;' class='btn btn-danger' value=''  id='refuse"+i+ "' name='delete' >sich weigern</button></form></td></tr>";
