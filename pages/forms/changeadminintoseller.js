...
function test4() {
    findUserByEmail1("SuperUser", function(arr) {
      ...
      console.log(s)
        if (arr.length == 0) {
            document.getElementById("emp").innerHTML = "<h3 style='text-align:center;color:gold;'>No admin Found</h3>"
        } else {
                    ...
                    .then((querySnapshot) => {
                        querySnapshot.forEach((doc) => {
                            ...
                            document.getElementById("delete" + i).addEventListener("click", e => {
                                ...
                                e.preventDefault();
                                let confirmAction = confirm("Are you sure to convert this admin into seller?");
                                if (confirmAction) {
